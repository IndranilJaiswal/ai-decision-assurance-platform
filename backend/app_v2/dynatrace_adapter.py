...
class DynatraceEvidenceAdapter:
    ...
    def collect(self, evidence_request: object) -> EvidenceRecord:
        ...
        if evidence_request.evidence_type == "response_time":
            return self._collect_metric_placeholder(
                evidence_request,
                reason=(
                    "Response time metric collection requires Dynatrace "
                    "metric query integration."
                ),
            )
        ...
        if evidence_request.evidence_type == "failure_rate":
            return self._collect_metric_placeholder(
                evidence_request,
                reason=(
                    "Failure rate metric collection requires Dynatrace "
                    "metric query integration."
                ),
            )
        ...
        if evidence_request.evidence_type == "dependency_call_success_rate":
            return self._collect_metric_placeholder(
                evidence_request,
                reason=(
                    "Dependency call success rate collection requires "
                    "Dynatrace service-flow metric integration."
                ),
            )
        ...
        if evidence_request.evidence_type == "dependency_response_time":
            return self._collect_metric_placeholder(
                evidence_request,
                reason=(
                    "Dependency response time collection requires Dynatrace "
                    "service dependency metric integration."
                ),
            )
    ...
    def _collect_active_problems(self, evidence_request: object) -> EvidenceRecord:
        ...
        If the provider does not expose problems, the evidence remains missing.
        """
    ...
    def _collect_metric_placeholder(
        ...
        It allows the Assurance Engine to demonstrate that a claim cannot be
        justified when required evidence is missing.
        """
